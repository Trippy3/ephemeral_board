...
A lightweight, casual real-time collaborative sticky-note board, built for ephemeral use only.
Runs on your local network and lets participants collaborate via an HTTP tunnel. Intended for short-lived sessions like workshops, retros, and brainstorms.

Stopping the server wipes all data.
Board state can be saved and restored as Markdown, which loosely covers carrying work between sessions. The Markdown format can also be generated by AI, so meeting minutes, web articles, and other summaries can be turned into a board.
