...
| `pnpm share` | `pnpm build` 後にサーバー + Cloudflare Tunnel を起動し公開 URL を表示。`--qr` で QR コードも描画。cloudflared の接続診断ログ（connected/disconnected/stderr/exit）は **既定で stderr に出力**（Quick Tunnel が不安定なときの調査用）、`--quiet` / `-s` で抑止 |
...
`scripts/share.ts` は `Tunnel.quick()` 経由で **TryCloudflare（アカウント無し版）** を立てている。
これは Cloudflare 自身が「uptime 保証なし・利用規約遵守の前提でしか使うな」と明言しているもので、
混雑時のエッジ切断や `*.trycloudflare.com` のホスト不可達など、アプリ側からは打つ手がない不安定要因がある。
切れたときに何が起きたかを最低限拾えるよう `pnpm share` は `--verbose` 相当のログを既定で stderr に出している。
ノイズが嫌なときは `pnpm share --quiet`。
恒久対策が必要になったら Named Tunnel（要 Cloudflare アカウント + ドメイン）への移行を検討する。
